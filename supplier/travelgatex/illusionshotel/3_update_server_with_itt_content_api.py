...
def send_batches(ids, supplier_code, source_name):
    remaining_ids = ids.copy()

    for i in range(0, len(ids), 2):
        batch = ids[i : i + 2]
        payload = json.dumps({"supplier_code": supplier_code, "hotel_id": batch})
...
# Send new IDs first
send_batches(new_ids, SUPPLIER, "new_hotel_id_list")
