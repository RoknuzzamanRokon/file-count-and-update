...
# Define paths
a_path = os.path.join(
    project_root, "all_hotel_count_id_list", "hyperguestdirect_hotel_id_list.txt"
)
...
explicit_folder = (
    "/var/www/Storage-Contents/Hotel-Supplier-Raw-Contents/hyperguestdirect"
)
explicit_folder = os.path.abspath(explicit_folder)
if os.path.isdir(explicit_folder):
    default_source_folder = explicit_folder
else:
    default_source_folder = os.path.join(project_root, "hyperguestdirect")

# Allow overriding the source folder via environment variable `hyperguestdirect_JSON_FOLDER`
source_folder = os.environ.get("HYPERGUESTDIRECT_JSON_FOLDER", default_source_folder)
