...
# Save directory inside the supplier/hotelbeds folder
...
# Define paths
a_path = os.path.join(
    project_root, "all_hotel_count_id_list", "hotelbeds_hotel_id_list.txt"
)
...
explicit_folder = "/var/www/Storage-Contents/Hotel-Supplier-Raw-Contents/hotelbeds"
explicit_folder = os.path.abspath(explicit_folder)
if os.path.isdir(explicit_folder):
    default_source_folder = explicit_folder
else:
    default_source_folder = os.path.join(project_root, "hotelbeds")

# Allow overriding the source folder via environment variable `HOTELBEDS_JSON_FOLDER`
source_folder = os.environ.get("HOTELBEDS_JSON_FOLDER", default_source_folder)
...
# Helper sort key that sorts numeric ids numerically, others lexicographically
def sort_key_mixed(x):
    try:
        return (0, int(x))
    except Exception:
        return (1, x)
...
# Write new IDs to file, sorted
with open(c_path, "w", encoding="utf-8") as f:
    for id in sorted(c, key=sort_key_mixed):
        f.write(id + "\n")
...
with open(d_path, "w", encoding="utf-8") as f:
    for id in sorted(only_update, key=sort_key_mixed):
        f.write(id + "\n")
