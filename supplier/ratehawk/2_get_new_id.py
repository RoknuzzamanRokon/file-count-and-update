...
a_path = os.path.join(
    ...
    "master_ratehawk_hotel_id_list_n.txt",
)

b_path = os.path.join(save_dir, "update_hotel_id_list.txt")
c_path = os.path.join(save_dir, "new_hotel_id_list.txt")

# Folder containing RateHawk JSON files
source_folder = "/var/www/Storage-Contents/Hotel-Supplier-Raw-Contents/ratehawkhotel"


def build_master_file(json_folder, output_file):
    """Create master_ratehawk_hotel_id_list_n.txt from JSON filenames."""
